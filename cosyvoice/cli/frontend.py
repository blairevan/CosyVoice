...
class CosyVoiceFrontEnd:
    ...
    def _extract_text_token(self, text):
        if isinstance(text, Generator):
            ...
            text_token_len = torch.tensor(
                [text_token.shape[1]], dtype=torch.int32).to(self.device)
    ...
    def _extract_speech_token(self, speech):
        assert speech.shape[1] / \
            16000 <= 30, 'do not support extract speech token for audio longer than 30s'
        ...
        speech_token_len = torch.tensor(
            [speech_token.shape[1]], dtype=torch.int32).to(self.device)
    ...
    def _extract_speech_feat(self, speech):
        speech_feat = self.feat_extractor(speech).squeeze(
            dim=0).transpose(0, 1).to(self.device)
        speech_feat = speech_feat.unsqueeze(dim=0)
        speech_feat_len = torch.tensor(
            [speech_feat.shape[1]], dtype=torch.int32).to(self.device)
    ...
    def text_normalize(self, text, split=True, text_frontend=True):
        ...
        if self.use_ttsfrd:
            ...
            if contains_chinese(text):
                ...
                # 不把句末 、 改成 。，与 split_paragraph 中不以 、 切分一致
                text = re.sub(r'[，,]+$', '。', text)
    ...
    def frontend_instruct2(self, tts_text, instruct_text, prompt_speech_16k, resample_rate, zero_shot_spk_id):
        model_input = self.frontend_zero_shot(
            tts_text, instruct_text + '<|endofprompt|>', prompt_speech_16k, resample_rate, zero_shot_spk_id)
        del model_input['llm_prompt_speech_token']
        del model_input['llm_prompt_speech_token_len']
        return model_input
